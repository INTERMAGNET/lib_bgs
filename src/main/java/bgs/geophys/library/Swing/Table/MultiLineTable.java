...
public class MultiLineTable extends JTable {
  ...
  public int getHeight(String text, int width) {
    ...
    int numLines = 1;
    // SMF: following line added - without it you can get a NullPointerException
  ...
  public int getRowHeight() {
    // SMF: following code changed - java uses GetRowHeight() when you call Table.SetNRows()
    // so we need to do something here - original code commented out at bottom of function
    int count, height;
    
    height=0;
    for (count=0; count<getRowCount(); count++)
        height += getRowHeight (count);
    if (getRowCount() > 0) height /= getRowCount ();
    return height;
...
//    return -1;
  ...
  public int getRowHeight(int row) {
    ...
    if (font == null) fontHeight = 0;
    else fontHeight = getFontMetrics(font).getHeight();
    height = fontHeight;
    i = 0;
    Object obj;
    String str;
	...
	height = Math.max(height, getHeight(str, colWidth));
      }
      i++;
    }
    return height;
