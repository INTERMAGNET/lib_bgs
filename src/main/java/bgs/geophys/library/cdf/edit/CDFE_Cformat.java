...
public class CDFE_Cformat {
  ...
  */
   ...
   private static CDFE_Cformat instance;
   private int width;
   ...
   private CDFE_Cformat() {

   }

   public static CDFE_Cformat getInstance(String s) {
      if (instance == null)
	instance = new CDFE_Cformat();
      instance.reset();
      instance.setup(s);
      return instance;
   }
  ...
  */
   ...
   public static void print(java.io.PrintStream s, String fmt, double x) {
      s.print(CDFE_Cformat.getInstance(fmt).form(x));
   }
  ...
  */
   ...
   */
   ...
   public String form(double x) {
      ...
      if (x < 0) { x = -x; s = -1; }
      if (fmt == 'f')
         r.append(fixed_format(x));
      else if (fmt == 'e' || fmt == 'E' || fmt == 'g' || fmt == 'G')
         r.append(exp_format(x));
      else if (fmt == 'd' || fmt == 'i') {
	long yy;
         if (s == -1 && x >= 1.0) { // Handling -1.0 and below
	    yy = (long) -x;
            r.append(("" + yy).substring(1));
         }
         else {
	    if (s == -1 && x < 1.0) s = 1;	// Changing -0.* to 0 
	    yy = (long) x; 
            r.append(yy); 
         }
      }
   ...
   */
   ...
   public String form(long x) {
      ...
      if (fmt == 'd' || fmt == 'i') {
         ...
         r.append(convert(x, 3, 7, "01234567"));
      else if (fmt == 'x')
         r.append(convert(x, 4, 15, "0123456789abcdef"));
      else if (fmt == 'X')
         r.append(convert(x, 4, 15, "0123456789ABCDEF"));
      else throw new java.lang.IllegalArgumentException();
   ...
   */
   ...
   private String pad(String r) {
      ...
      p.append(repeat(' ', width - r.length()));
      if (left_align) return pre + r + p.toString() + post;
      else return pre + p.toString() + r + post;
   }
   ...
   private String sign(int s, String r) {
      ...
      if (s < 0) p.append("-"); 
         ...
         if (show_plus) p.append("+");
         else if (show_space) p.append(" ");
      }
      else {
         if (fmt == 'o' && alternate && r.length() > 0 && r.charAt(0) != '0') p.append("0");
         else if (fmt == 'x' && alternate) p.append("0x");
         else if (fmt == 'X' && alternate) p.append("0X");
      }
      int w = 0;
      if (leading_zeroes) 
      ...
      if (fmt == 'd' || fmt == 'i')
	return p.append(r).toString();
      else
        return p.append(repeat('0', w - p.length() - r.length())).append(r).toString();
   }
   ...
   private String fixed_format(double d) {
      ...
      double factor = 1;
      StringBuffer sleading_zeroes = new StringBuffer();
      for (int i = 1; i <= precision && factor <= 0x7FFFFFFFFFFFFFFFL; i++) {
         factor *= 10; 
         sleading_zeroes.append("0"); 
      }
      ...
      z.append(sleading_zeroes.toString()).append(l);
      y.append(".").append(z.toString().substring(z.length() - precision, z.length()));
      ...
      return y.insert(0, whole).toString();
   }
   ...
   private String exp_format(double d) {
      ...
      return f.append(p.toString().substring(p.length() - 2, p.length())).toString();
   }
