...
/**
 * Class that holds a single instance of port forwarding
...
public class sshPortForward 
{
     ...
     * similar to -L option to ssh, but this method chooses the local port
     ...
     */
    public sshPortForward(Session session, String fwd_rhost, int fwd_rport)
    throws JSchException
    {
        ...
        while (! connected)
        {
            try
            {
                session.setPortForwardingL (fwd_lport,  fwd_rhost, fwd_rport);
                connected = true;
            }
     ...
     * @throws JSchException if there was an ssh connection/authentication error
     */
    public sshPortForward(Session session, int fwd_lport, String fwd_rhost, int fwd_rport)
    throws JSchException
    {
        session.setPortForwardingL (fwd_lport,  fwd_rhost, fwd_rport);
    ...
    public void close ()
    {
        try
        {
            session.delPortForwardingL (fwd_lport);
        }
        catch (Exception e) { }
    }
